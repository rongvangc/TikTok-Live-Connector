import { t as TikTokLiveConnection } from "./lib-CZoAbYHv.js";
		...
		case "WebcastLinkMicBattle":
			originalObject = simplify((webcastObject) => {
				...
				Object.values(webcastObject.anchorInfo).forEach((anchor) => {
					if (anchor.user) battleUsers.push(getUserAttributes(anchor.user));
				});
...
var WebcastPushConnection = class extends TikTokLiveConnection {
	async processProtoMessageFetchResult(fetchResult) {
		fetchResult.messages.forEach((message) => {
			this.emit("rawData", message.type, message.payload);
		});
		fetchResult.messages.forEach((message) => {
			let simplifiedObj = simplifyObject(message.type, message.decodedData?.data || {});
			this.emit("decodedData", message.type, simplifiedObj, message.payload);
			switch (message.type) {
				...
				case "WebcastBarrageMessage":
					...
					if (simplifiedObj.content?.displayType?.toLowerCase()?.includes("ttlive_superfan") || simplifiedObj.displayType?.toLowerCase()?.includes("ttlive_superfan")) this.emit("superFan", simplifiedObj);
					break;
